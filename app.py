...
def render_net_sales_view(df: pd.DataFrame) -> None:    
    st.markdown('<div class="section-title">Ventas Netas</div>', unsafe_allow_html=True)    
    
    # Identificar la columna del cliente
    customer_label = "CustomerName" if "CustomerName" in df.columns else "CustomerID"    
    
    # Agrupación 1: Ventas por categoría (Corregido)
    sales_by_category = (        
        df.groupby("Category", as_index=False)["net_sales"]        
        .sum()        
        .sort_values("net_sales", ascending=False)    
    ...
    # Agrupación 2: Resumen por cliente
    customer_summary = (        
        df.groupby(customer_label, as_index=False)        
        .agg(            
            transactions=("TransactionID", "nunique"),            
            net_sales=("net_sales", "sum"),            
            profit=("profit", "sum")        
    ...
    with chart_col2:        
        fig_top_customers = px.bar(            
            top_10_customers.sort_values("net_sales"),            
            x="net_sales",            
            y=customer_label,            
            orientation="h",            
            title="Top 10 clientes por ventas netas"        
    ...
    # Tabla resumen enfocada solo en el Top 10
    st.markdown('<div class="section-title" style="font-size: 1.1rem;">Resumen detallado (Top 10 Clientes)</div>', unsafe_allow_html=True)    
    
    display_table = top_10_customers.rename(columns={
...
try:    
    ...
    elif selected_view == "Ventas Netas":  
        render_net_sales_view(df)
    else:        
